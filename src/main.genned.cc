...
public:
  ...
  // Direct view into the contacts memory - no copies
  // val getContactsView() {
  //   if (!d || !d->contact)
  //     return val::null();

  //   // Return a view directly into contact memory
  //   return val(typed_memory_view(d->ncon * sizeof(mjContact),
  //                                reinterpret_cast<uint8_t *>(d->contact)));
  // }
    // Direct access to the contacts array without copying
    // const std::vector<mjContact>& getContacts() {
    //     if (!d) {
    //         throw std::runtime_error("Invalid mjData pointer");
    //     }
    //     // Update the cached vector only when needed
    //     contacts.assign(d->contact, d->contact + d->ncon);
    //     return contacts;
    // }

        // Optimized getContacts function
val getContacts() {
    if (!d || !d->contact || d->ncon == 0) {
        static val emptyContacts = val::array(); // Persistent empty array
        return emptyContacts;
    }

    static val contacts = val::array(); // Persistent array to reuse across calls
    contacts.set("length", d->ncon);    // Pre-size the array to avoid dynamic resizing

    for (int i = 0; i < d->ncon; i++) {
        const mjContact& c = d->contact[i];

        // Create a new object for the current contact
        val contact = val::object();
        contact.set("dist", c.dist);
        contact.set("geom1", c.geom1);
        contact.set("geom2", c.geom2);
        contact.set("pos", val(typed_memory_view(3, c.pos))); // Directly expose position

        // Set the object at the correct index
...
EMSCRIPTEN_BINDINGS(mujoco_wasm) {
    value_object<mjContact>("mjContact")
        ...
        .field("geom1", &mjContact::geom1);
    
    // Register the vector of pointers
  ...
  class_<State>("State")
      ...
      // .function("getContactsView", &State::getContactsView)
        .function("getContacts", &State::getContacts, allow_raw_pointers());
  ...
  class_<Simulation>("Simulation")
      ...
      .function("_mulVecMatVec", &Simulation::_mulVecMatVec,
                allow_raw_pointers())
      ...
      .function("_encodePyramid", &Simulation::_encodePyramid,
                allow_raw_pointers())
      .function("_decodePyramid", &Simulation::_decodePyramid,
                allow_raw_pointers())
      ...
      .function("_standardNormal", &Simulation::_standardNormal,
                allow_raw_pointers())
      .function("_insertionSort", &Simulation::_insertionSort,
                allow_raw_pointers())
      .function("_Halton", &Simulation::_Halton)
      .function("_sigmoid", &Simulation::_sigmoid)
      .function("_transitionFD", &Simulation::_transitionFD,
                allow_raw_pointers())
  ...
  value_object<mjvPerturb>("mjvPerturb")
      .field("select",
             &mjvPerturb::select) // selected body id; non-positive: none
      .field("skinselect",
             &mjvPerturb::skinselect)       // selected skin id; negative: none
      .field("active", &mjvPerturb::active) // perturbation bitmask (mjtPertBit)
      .field(
          "active2",
          &mjvPerturb::active2) // secondary perturbation bitmask (mjtPertBit)
      .field("refpos",
             &mjvPerturb::refpos) // desired position for selected object
      .field("refquat",
             &mjvPerturb::refquat) // desired orientation for selected object
      .field("localpos",
             &mjvPerturb::localpos) // selection point in object coordinates
      .field("scale", &mjvPerturb::scale) // relative mouse motion-to-space
                                          // scaling (set by initPerturb)
      ;
  ...
  value_object<mjLROpt>("mjLROpt")
      ...
      .field("accel",
             &mjLROpt::accel) // target acceleration used to compute force
      .field("maxforce", &mjLROpt::maxforce) // maximum force; 0: no limit
      .field(
          "timeconst",
          &mjLROpt::timeconst) // time constant for velocity reduction; min 0.01
      .field(
          "timestep",
          &mjLROpt::timestep) // simulation timestep; 0: use mjOption.timestep
      .field("inttotal", &mjLROpt::inttotal) // total simulation time interval
      .field("interval",
             &mjLROpt::interval) // evaluation time interval (at the end)
      .field("tolrange",
             &mjLROpt::tolrange); // convergence tolerance (relative to range)
  ...
  value_object<mjOption>("mjOption")
      ...
      .field(
          "impratio",
          &mjOption::impratio) // ratio of friction-to-normal contact impedance
      .field("tolerance", &mjOption::tolerance) // main solver tolerance
      .field("noslip_tolerance",
             &mjOption::noslip_tolerance) // noslip solver tolerance
      //.field("gravity"           , &mjOption::gravity)           //
      // gravitational acceleration .field("wind"              ,
      // &mjOption::wind)
      //// wind (for lift, drag and viscosity) .field("magnetic"          ,
      //&mjOption::magnetic)          // global magnetic flux
      ...
      .field("integrator",
             &mjOption::integrator) // integration mode (mjtIntegrator)
      ...
      .field("iterations",
             &mjOption::iterations) // maximum number of main solver iterations
      .field("noslip_iterations",
             &mjOption::noslip_iterations) // maximum number of noslip solver
                                           // iterations
      .field(
          "disableflags",
          &mjOption::disableflags) // bit flags for disabling standard features
      .field(
          "enableflags",
          &mjOption::enableflags); // bit flags for enabling optional features
